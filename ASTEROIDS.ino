...
void asteroid() {
  for (uint8_t t = 0; t < asteroidCount; t++) {
    ...
    switch (asteroidDraw[t]) {
      case 0: arduboy.drawBitmap(asteroids[t][0] - 7, asteroids[t][2] - 7, asteroidS0, 16, 16, WHITE); break;
      case 1: arduboy.drawBitmap(asteroids[t][0] - 7, asteroids[t][2] - 7, asteroidS1, 16, 16, WHITE); break;
      case 2: arduboy.drawBitmap(asteroids[t][0] - 7, asteroids[t][2] - 7, asteroidS2, 16, 16, WHITE); break;
      case 3: arduboy.drawBitmap(asteroids[t][0] - 7, asteroids[t][2] - 7, asteroidS3, 16, 16, WHITE); break;
    }
...
void rock() {
  for (uint8_t r = 0; r < rockCount; r++) {
    ...
    switch (rockDraw[r]) {
      case 0: arduboy.drawBitmap(rocks[r][0] - 3, rocks[r][2] - 3, rockS0, 8, 8, WHITE); break;
      case 1: arduboy.drawBitmap(rocks[r][0] - 3, rocks[r][2] - 3, rockS1, 8, 8, WHITE); break;
      case 2: arduboy.drawBitmap(rocks[r][0] - 3, rocks[r][2] - 3, rockS2, 8, 8, WHITE); break;
      case 3: arduboy.drawBitmap(rocks[r][0] - 3, rocks[r][2] - 3, rockS3, 8, 8, WHITE); break;
    }
...
void pebble() {
  for (uint8_t p = 0; p < pebbleCount; p++) {
    ...
    switch (pebbleDraw[p]) {
      case 0: arduboy.drawBitmap(pebbles[p][0] - 2, pebbles[p][2] - 2, pebbleS0, 8, 8, WHITE); break;
      case 1: arduboy.drawBitmap(pebbles[p][0] - 2, pebbles[p][2] - 2, pebbleS1, 8, 8, WHITE); break;
      case 2: arduboy.drawBitmap(pebbles[p][0] - 2, pebbles[p][2] - 2, pebbleS2, 8, 8, WHITE); break;
      case 3: arduboy.drawBitmap(pebbles[p][0] - 2, pebbles[p][2] - 2, pebbleS3, 8, 8, WHITE); break;
    }
...
void radar() {
  ...
  for (uint8_t r = 0; r < rockCount; r ++) {
    if (abs(rocks[r][0] - shipX) < 5 && abs(rocks[r][2] - shipY) < 5) {
      ...
      pebbles[pebbleCount][4] = random(0, 4);
      pebbleCount = pebbleCount + 1;
      for (uint8_t c = r; c < rockCount - 1; c++)
        for (uint8_t d = 0; d < 4; d++)
          rocks[c][d] = rocks[c + 1][d];
  ...
  if (asteroidCount == 0 && rockCount == 0 && pebbleCount == 0) {
    ...
    if (tick <= 0) {
      ...
      for (int8_t s = 0; s < level; s++) {
        ...
        asteroids[asteroidCount][4] = random(0, 4);
        asteroidCount = asteroidCount + 1;
      }
    }
  }
...
void loop() {
  ...
  else if (state == 0) arduboy.setFrameRate(20);
