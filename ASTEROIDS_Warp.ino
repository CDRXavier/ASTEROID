...
char tick;
bool turnC = false;
enum class ProgState { mainMenu, Game,  HighScoreEntry, HighScore, Pause, EraseHighScore};
char initials[3];
//ProgState ProgState = ProgState::MainMenu;
  ...
  switch (ProgState) {
    ...
    case ProgState::Game: {
        if (life > 0) {
          ...
          if (arduboy.pressed(A_BUTTON) && released) {
            released = false;
            ProgState = ProgState::Pause;
          }
        } else {
          delay(3000);
          resetField();
          ProgState = ProgState::HighScoreEntry;
        }
      }
      break;
    case ProgState::HighScoreEntry: {
        char tmpInitials[3];
    ...
    case ProgState::HighScore:
      ...
      if (arduboy.pressed(LEFT_BUTTON) && released) {
        released = false;
        ProgState = ProgState::EraseHighScore;
      }
      break;
